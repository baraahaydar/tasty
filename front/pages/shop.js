...
//   return (
//     <center style={{marginTop:65}}>
//         {state.map((l) => (
//           <button key={l.id} onClick={() => someEventHandler(l.id)}>{l.name}</button>
//         ))}
//     </center>
//   );
// }
...
import { ArrowBack, LocalDining } from "@material-ui/icons";
...
const useStyles = makeStyles((theme) => ({
  ...
  shop_2: {
    backgroundImage: `URL('./TRUESSS.png') `,
      ...
      <div>
        ...
        {state.map((l) => (
          <Box
            ...
            classes={classes}
          >
            <Button onClick={() => someEventHandler(l.id)}>SHOP NOW</Button>
          </Box>
        ))}
</div>
    </div>
